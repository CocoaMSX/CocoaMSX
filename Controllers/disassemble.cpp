...
UInt16 Disassembly::dasm(UInt16 pc, char* dest)
{
    dest[0] = 0;
...
Disassembly::Disassembly(SymbolInfo* symInfo/*, Breakpoints* breakpts*/) :
linePos(0), lineCount(0), currentLine(-1), programCounter(0), firstVisibleLine(0),
...
void Disassembly::setCursor(UInt16 address)
{
    for (int i = lineCount - 1; i >= 0; i--) {
...
void Disassembly::updateContent(UInt8* memory, UInt16 pc)
{
    int addr = 0;
    //breakpoints->clearRuntoBreakpoint();
    ...
    for (; addr < pc; ) {
        ...
        strncat (lineInfo[lineCount].dataText,"            ",12-lineInfo[lineCount].dataTextLength);
        addr += len;
        lineCount++;
    }
    if (addr > pc) addr = pc;
